...
def prepara_datos(ruta):
    ...
    return datos, anos_ultimo_csv


def verifica_datasets_anteriores():
    ...
    return ultimo_ano_guardado


def guarda_nuevo_ano_completo_si_existe(datos, ultimo_ano_guardado):
...
def concatena_anteriores_con_ultimo_ano(datos):
    datos.fecha = datos.fecha.map(
        lambda x: int(str(x.date()).replace("-", "")))
...
if __name__ == "__main__":
    ruta_ultimo_dataset = input("Ingresar ruta del dataset: ")
    datos, anos_ultimo_csv = prepara_datos(ruta_ultimo_dataset)
    ultimo_ano_guardado = verifica_datasets_anteriores()
    datos = guarda_nuevo_ano_completo_si_existe(datos, ultimo_ano_guardado)
    concatena_anteriores_con_ultimo_ano(datos)
